...
export const authConfig = {
  ...
  callbacks: {
    signIn: async ({ user, account, profile, credentials, email }) => {
      ...
      const signup = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/signup`, {
        method: "POST",
        headers: {
    ...
    authorized({ auth, request }) {
      ...
      if (request.nextUrl.pathname === "/auth/signin" && isLoggedIn) {
        return Response.redirect(new URL("/dashboard", request.nextUrl));
      } else if (request.nextUrl.pathname === "/auth/signup" && isLoggedIn) {
        return Response.redirect(new URL("/dashboard", request.nextUrl));
      }
